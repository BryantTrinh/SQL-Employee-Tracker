		...
		prompt([
			...
			.then(([rows]) => {
				...
				if (employees.length === 0) {
					console.log(
						'The selected employee has no direct manager that they report to'
					);
				} else {
					console.table(employees);
				}
		...
		prompt([
			{
				...
				name: 'roleId',
				message:
					'Which role do you want to remove? This will remove eployees as well',
				choices: roleChoices,
			},
		])
...
function removeDepartment() {
	db.findAllDepartments().then(([rows]) => {
		...
		prompt({
			...
			name: 'departmentId',
			message:
				'Which department do you want to remove? This will remove their roles and employees',
			choices: departmentChoices,
		})
